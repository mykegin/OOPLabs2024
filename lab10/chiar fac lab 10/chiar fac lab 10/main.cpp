...
public:
    ...
    {
        return "Este indexul out of range cumetre, esti nesimtit?";
    }
};
...
private:
    int Current;
    int Size; // mai adaugati si alte date si functii necesare pentru iterator
    T** PtrCurent;
public:
    ArrayIterator()
    {
    ...
    {
        return (Current == other.Current && PtrCurent == other.PtrCurent);
    }
    bool operator!=(ArrayIterator<T>& other)
    {
        return !(Current == other.Current && PtrCurent == other.PtrCurent);
    ...
    T* GetElement()
    {
        if (PtrCurent != nullptr && Current < Size)
            return *PtrCurent;
        return nullptr;
    }
};
